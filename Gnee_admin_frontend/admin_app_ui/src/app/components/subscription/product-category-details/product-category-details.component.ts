...
export class ProductCategoryDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    sessionStorage.setItem('prod_cat_id',this.id)
    switch(this.mode)
    {
      case 'add':
        console.log('add')
  ...
  getProdCategoryData(){ //solved
    var getURL ="/api/prodcat/view/"+sessionStorage.getItem('prod_cat_id')
    const headers = {'Authorization' : this.token}
    this.http.get(getURL ,{headers,observe: 'response'})
    .subscribe(response => {
      ...
      this.prod_cat_id=detail[0]['prod_cat_id']
      this.prod_cat_name=detail[0]['prod_cat_name']
  ...
  updateProdCategoryData(){
    var putURL ="/api/prodcat/update/"+sessionStorage.getItem('prod_cat_id')
    const headers = {'Authorization' : this.token}
    var country_update=this.productCategoryForm.value['country']
    var raw_data= {
      "is_active": this.productCategoryForm.value['is_active'],
      "prod_cat_name": this.productCategoryForm.value['prod_cat_name'],
          }
    this.http.put(putURL,raw_data,{headers,observe: 'response'})
    .subscribe(response => {
      console.log(response)
      this.router.navigateByUrl('administrator/subscription/product-category/details/'+this.id+'/view')  
      this.mode='view'
    });
  }

  createProdCategoryData(){
    var postURL ="/api/prodcat/add"
    const headers = {'Authorization' : this.token}
    console.log(this.country_dict)
    console.log(this.country_dict[this.productCategoryForm.value['country']])
    var raw_data= {
      "is_active": this.productCategoryForm.value['is_active'],
      "prod_cat_name": this.productCategoryForm.value['prod_cat_name'],
      "updated_by":3
      ...
      console.log(raw_data)
        ...
        .subscribe(response => {
          console.log(response)
          this.router.navigateByUrl('administrator/subscription/product-category')
        });
  }
  ...
  intializeForm(){ //solved
    this.productCategoryForm=this.fb.group({
      ...
      prod_cat_name:['',Validators.required],
  ...
  pushFormValues(){
    ...
    this.productCategoryForm.patchValue({
      ...
      prod_cat_name:this.prod_cat_name,
      updated_by:this.updated_by
    });
    console.log(this.productCategoryForm.value['prod_cat_name'])
  }

  onEdit(){ //solved
    this.router.navigateByUrl('administrator/subscription/product-category/details/'+this.id+'/edit')  
    this.mode='edit'
  }

  onSave(){
    this.updateProdCategoryData()
  }
